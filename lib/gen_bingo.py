...
def gen_bingo(dbname, user, filename, width, height, bg_color, line_color, text_color, font_path, free_space=''):
    ...
    while it < 25:
        if(it == 12):
            ...
            extra_len = 0
            try:
                cur.execute("SELECT COUNT(word) FROM bingo WHERE main IS TRUE AND used IS FALSE;")
                main_len = cur.fetchone()[0]
            except:
                main_len = 0
            try:
                cur.execute("SELECT COUNT(word) FROM bingo WHERE main IS FALSE AND used IS FALSE;")
            ...
            if(main_len > 0 and roll < 80):
                cur.execute("SELECT word FROM bingo WHERE main IS TRUE AND used IS FALSE;")
                new_word = cur.fetchall()[rr(main_len)][0]
                board_values.append(new_word)
                cur.execute("UPDATE bingo SET used=TRUE WHERE word='{0}';".format(new_word))
                it += 1
            if(extra_len > 0 and roll >= 80):
                cur.execute("SELECT word FROM bingo WHERE main IS FALSE AND used IS FALSE;")
